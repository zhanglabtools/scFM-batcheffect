...
# Reads models from config.yaml and runs integration for each
...
if [ -z "$DATASET_DIR" ]; then
    echo "Usage: $0 <dataset_dir> [result_dir]"
    exit 1
fi
...
echo "=========================================="
echo "Integration from config.yaml"
echo "=========================================="
...
echo "Models to integrate: ${MODEL_ARRAY[@]}"
...
for i in "${!MODEL_ARRAY[@]}"; do
    ...
    if python "$SCRIPT_DIR/integrate.py" \
        --dataset "$DATASET_DIR" \
        --model "$model" \
...
if [ ${#SUCCESS_MODELS[@]} -gt 0 ]; then
    echo -e "${GREEN}✓ Successfully integrated (${#SUCCESS_MODELS[@]}):${NC}"
...
if [ ${#FAILED_MODELS[@]} -gt 0 ]; then
    ...
    echo -e "${GREEN}All models integrated successfully!${NC}"
    exit 0
fi
