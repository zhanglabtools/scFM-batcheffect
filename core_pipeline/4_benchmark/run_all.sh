...
# Reads models from config.yaml and runs benchmark for each
...
echo "=========================================="
echo "Benchmark from config.yaml"
echo "=========================================="
...
echo "Models to benchmark: ${MODEL_ARRAY[@]}"
...
for i in "${!MODEL_ARRAY[@]}"; do
    ...
    if python "$SCRIPT_DIR/benchmark.py" \
        --dataset "$DATASET_DIR" \
        --model "$model" \
...
if [ ${#SUCCESS_MODELS[@]} -gt 0 ]; then
    echo -e "${GREEN}✓ Successfully benchmarked (${#SUCCESS_MODELS[@]}):${NC}"
...
if [ ${#FAILED_MODELS[@]} -gt 0 ]; then
    ...
    echo -e "${GREEN}All models benchmarked successfully!${NC}"
    exit 0
fi
